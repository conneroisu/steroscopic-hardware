...
var defaultLogger = atomic.Pointer[ChannelHandler]{}
var defaultChannel = atomic.Pointer[chan LogEntry]{}

func init() {
	ch := make(chan LogEntry, 100)
	defaultChannel.Store(&ch)
	defaultLogger.Store(NewChannelHandler(ch, slog.LevelDebug))
}

// SetDefaultSlog sets the default logger to a slog.Logger.
func SetDefaultSlog() {
	channelHandler := NewChannelHandler(*defaultChannel.Load(), slog.LevelInfo)
...
func Default() chan LogEntry {
	return *defaultChannel.Load()
}
