...
func (p *props) decodeProps(buf []byte) error {
	...
	if d > (9 * 5 * 5) {
		return &HeaderError{
			msg: fmt.Sprintf("invalid lc, lp, pb identifier in header: %d", d),
		}
	}
	...
	if p.litContextBits > kNumLitContextBitsMax || p.litPosStateBits > 4 || p.posStateBits > kNumPosStatesBitsMax {
		return &HeaderError{
			msg: fmt.Sprintf("invalid lc, lp, pb value in header: %d", d),
		}
	}
	for i := range 4 {
...
func (z *decoder) doDecode() error {
	var state, rep0, rep1, rep2, rep3 uint32
	var nowPos uint64
	var prevByte byte
	...
	for z.unpackSize < 0 || int64(nowPos) < z.unpackSize {
		...
		if z.rd.decodeBit(z.matchDecoders, state<<kNumPosStatesBitsMax+posState) == 0 {
				...
				if posSlot >= kStartPosModelIndex {
					...
					if posSlot < kEndPosModelIndex {
						...
						if int32(rep0) < 0 {
							...
							return &StreamError{
								msg: fmt.Sprintf("invalid rep0 value: %d", rep0),
							}
						}
					}
				} else {
			...
			if uint64(rep0) >= nowPos {
				return &StreamError{
					msg: fmt.Sprintf("invalid rep0 value: %d", rep0),
				}
			}
			if rep0 >= z.dictSizeCheck {
				return &StreamError{
					msg: fmt.Sprintf("invalid rep0 value (is greater than or equal to dictSizeCheck): %d", rep0),
				}
			}
